...
export default function VisualEffect({ mood }: VisualEffectProps) {
  ...
  useEffect(() => {
    // Return early if component is not mounted on client yet
    if (!isMounted) return;
    ...
    for (let i = 0; i < particleSettings.count; i++) {
      particles.push({
        ...
        size:
          Math.random() *
            (particleSettings.size.max - particleSettings.size.min) +
          particleSettings.size.min,
        speed:
          Math.random() *
            (particleSettings.speed.max - particleSettings.speed.min) +
          particleSettings.speed.min,
        opacity:
          Math.random() *
            (particleSettings.opacity.max - particleSettings.opacity.min) +
          particleSettings.opacity.min,
      });
    }
