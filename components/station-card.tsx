...
export default function StationCard({ station, isActive, isPlaying, onClick }: StationCardProps) {
  ...
  useEffect(() => {
    ...
    const drawChillhopEffect = () => {
      ...
      for (let i = 0; i < waveCount; i++) {
        ...
        for (let x = 0; x < canvas.width; x += 5) {
          const y = Math.sin(x * 0.01 + time + i) * waveHeight + canvas.height / 2
          ctx.lineTo(x, y)
        }
    ...
    const drawLofiGirlEffect = () => {
      ...
      for (let i = 5; i > 0; i--) {
        ...
        // Draw groove line
        ctx.beginPath()
        ctx.arc(centerX, centerY, radius, rotation % (Math.PI * 2), (rotation + Math.PI) % (Math.PI * 2))
    ...
    const drawJazzVibesEffect = () => {
      ...
      for (let i = 0; i < noteCount; i++) {
        ...
        const size = 3 + Math.sin(time + i * 0.5) * 1

        // Draw note
        ctx.beginPath()
        ctx.arc(x, y, size, 0, Math.PI * 2)
        ctx.fillStyle = `${baseColor}${Math.floor(50 + Math.sin(time + i) * 30).toString(16)}`
        ctx.fill()
    ...
    const drawNatureEffect = () => {
      ...
      for (let i = 0; i < leafCount; i++) {
        ...
        ctx.beginPath()
        ctx.ellipse(x, y, size, size * 2, Math.PI * 0.25 * Math.sin(time * 0.5 + i), 0, Math.PI * 2)
        ctx.strokeStyle = `${baseColor}${Math.floor(40 + Math.sin(time + i) * 20).toString(16)}`
        ctx.lineWidth = 1
        ctx.stroke()
      }
    ...
    // Select effect based on station id
    switch (station.id) {
      case "chillhop":
        drawChillhopEffect()
        break
  ...
  return (
    <div
      className={cn(
        ...
        isActive ? "bg-black/40 border-2 border-white/20" : "bg-black/30 border border-white/10 hover:bg-black/40",
      )}
      onClick={onClick}
    >
